...
def event_to_array(event, dim):
    ...
    return 255 - vox_ec / vox_ec.max() * 255
...
def trans_to_images(path):
    ...
    folderPath = os.path.join(folderPath, "Caltech101")
    ...
    for className in classList:
        ...
        for index, spikingImage in enumerate(allSpikingImages):
            ...
            eventData = ev.loadaerdat(spikingImagePath, camera='N101')
            image = torch.tensor(eventData.toArray('xytp').astype(np.int32))
            image = event_to_array(image, [180, 240]).unsqueeze(0).repeat(3,1,1).permute(1,2,0).numpy()
            image = Image.fromarray(np.uint8(image))
            image.save(os.path.join(imagePath, os.path.splitext(spikingImage)[0] + ".png"))

if __name__ == '__main__':
    path = "/home/xjy/datasets/NCALTECH101"
